<template>
  <div>
    ...
    <el-table v-loading="loading" :data="rows" stripe border>
      ...
      <el-table-column label="模型类型" min-width="120" show-overflow-tooltip>
        <template #default="{ row }">
      ...
      <el-table-column label="绑定模型" min-width="200" show-overflow-tooltip>
        <template #default="{ row }">
          ...
          <template v-else>
            <el-tag
              ...
              :key="m.modelId"
              size="small"
              :type="m.primary ? 'success' : 'info'"
              class="model-tag"
            >
              {{ m.modelName || m.modelCode }}{{ m.primary ? "（主）" : "" }}
              <span class="priority-hint">P{{ m.priority }}</span>
            </el-tag>
          </template>
        </template>
      </el-table-column>
...
const { options: strategyOptions, loadOptions: loadStrategyOptions } = useEnums("strategy_type");
